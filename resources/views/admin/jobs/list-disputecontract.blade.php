            ...
            <div class="x_content">
                <div class="row">
                    <div class="col-sm-12">
                        ...
                        <table id="dispute_contract_datatable" class="table table-striped table-bordered"
                            style="width:100%">
                            <thead>
                                <tr>
...
@push('js')
    <script>
        $(function() {
            refreshTable();
            $('#dispute_contract_datatable').DataTable({
                processing: true,
                serverSide: true,
                ajax: "{{ route('contract.get.data') }}",
                columns: [{
                        data: 'contract.recruiter.name',
                        name: 'contract.recruiter.name'
                    },
                    {
                        data: 'contract.freelancer.name',
                ...
                rowCallback: function(row, data, index) {
                    if (data.status == 0) {
                        $("td:eq(2)", row).html("Started");
                    } else if (data.status == 1) {
                        $("td:eq(2)", row).html("Processing");
                    } else if (data.status == 2) {
                        $("td:eq(2)", row).html("Resolved");
                    }
        ...
        function refreshTable() {
            $('#dispute_contract_datatable').DataTable().destroy();
